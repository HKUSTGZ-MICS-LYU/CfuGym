...
def build_graph():
    ...
    graph.add_edge("ingest_input", "analyze_workload")
...
def run_agent(initial_state: CfuDesignState) -> CfuDesignState:
...
def fallback_ascii_graph() -> str:
    return "\n".join(
        [
            ...
            "  -> estimate_cost",
            "  -> review_and_route",
            "       | needs_iteration -> implement_hw_sw",
            "       | finish -> final_report",
            "  -> END",
            "",
        ]
...
def validate(state: CfuDesignState) -> CfuDesignState:
    ...
    for command in commands:
        ...
        if results[-1]["returncode"] != 0:
            break
    return {
        "validation_results": [*state.get("validation_results", []), *results],
        "errors": errors,
...
def estimate_cost(state: CfuDesignState) -> CfuDesignState:
    if state.get("skip_yosys", False):
        ...
        return {
            "cost_results": [*state.get("cost_results", []), result],
...
def review_and_route(state: CfuDesignState) -> CfuDesignState:
...
def route_after_review(state: CfuDesignState) -> str:
    return "iterate" if state.get("status") == "needs_iteration" else "finish"
