...
Both runners default to `--dry-run --skip-sim --skip-yosys` so LLM calls can be tested without applying generated patches or launching long hardware flows.

Useful overrides:

```bash
CFU_AGENT_MAX_ITERS=2 bash agents/examples/run_natural_language_llm.sh
CFU_AGENT_MUTATE=1 bash agents/examples/run_c_project_llm.sh
